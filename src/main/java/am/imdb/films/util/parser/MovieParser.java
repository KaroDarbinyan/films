...
public class MovieParser {
    ...
    public List<MovieDto> csvParser(MultipartFile csvFile) throws IOException {
        ...
        try (Reader reader = new BufferedReader(new InputStreamReader(csvFile.getInputStream()))) {
            List<MovieDto> parse = new CsvToBeanBuilder<MovieDto>(reader)
            ...
            return parse.stream()
                    .peek(movieDto -> {
                        movieDto.setGenres(mapToGenreDtoSet(movieDto.getGenreNames()));
                        movieDto.setCountries(mapToCountryDtoSet(movieDto.getCountryNames()));
                        movieDto.setLanguages(mapToLanguageDtoSet(movieDto.getLanguageNames()));
